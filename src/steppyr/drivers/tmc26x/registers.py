...
class Register(Datagram):
  ...
  def __init__(self, register_value=0, header=None,
    header_len=3, datagram_len=20):
    """
    header: header value, always the register address. Can be 2 or 3 bits.
    register_value: value of the register, not including header.
    header_len: the number of bits in the header. Usually 3, except for DRVCTRL.
    datagram_len: total number of bits in a complete datagram.
    """
    if header == None:
  ...
  def get(self, bitmask):
    return get_bits(self._data, bitmask)

  def get_all_values(self):
    events = {}
    for name, bitmask in self.bits.items():
      value = (self._data & bitmask) >> lsb(bitmask)
      events[name] = value
    return events
...
class DriverControlRegister(Register):
  ...
  def __init__(self, register_value=0, header=None,
    ...
    """
    super().__init__(register_value=register_value, header=header,
